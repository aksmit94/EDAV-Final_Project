...
```{r}
drg_grouped_by_year <-  data_na_removed %>% group_by(`DRG Definition`, `Year`,
                                                     `Provider State`) %>%
...
```{r}
total_cost_by_state <- drg_grouped_by_year %>% group_by(`Provider State`) %>% summarise(Total_Discharges = sum(Count_Discharge),
                                                                                        Total_Cost = sum(Sum_Amount))
total_cost_by_state$value <- total_cost_by_state$Total_Cost/total_cost_by_state$Total_Discharges
...
```{r}
state_choropleth(total_cost_by_state)
